...
class NavService(Node):
    ...
    def service_callback(self, request: ArucoNavigateTo.Request, response: ArucoNavigateTo.Response):
        ...
        send_goal_future.add_done_callback(lambda future: self.goal_response_callback(future, request.aruco_id, request.final_id))
    ...
    def goal_response_callback(self, future, aruco_id, final_id):
        ...
        get_result_future.add_done_callback(lambda future: self.get_result_callback(future, aruco_id, final_id))
    ...
    def get_result_callback(self, future, aruco_id, final_id):
        try:
            ...
            self.start_aruco_detection(aruco_id, final_id)
        except Exception as e:
            self.get_logger().error(f'Error in getting result: {e}')
            self.target_reached = False

    def start_aruco_detection(self, aruco_id, final_id):
